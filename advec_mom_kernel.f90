...
SUBROUTINE advec_mom_kernel(x_min,x_max,y_min,y_max,z_min,z_max,   &
  ...
  REAL(KIND=8), DIMENSION(x_min-2:x_max+3,y_min-2:y_max+3,z_min-2:z_max+2) :: node_flux
  REAL(KIND=8), DIMENSION(x_min-2:x_max+3,y_min-2:y_max+3,z_min-2:z_max+2) :: node_mass_post
  REAL(KIND=8), DIMENSION(x_min-2:x_max+3,y_min-2:y_max+3,z_min-2:z_max+2) :: node_mass_pre
      ...
      DO k=y_min,y_max+1
        DO j=x_min-2,x_max+2
          ...
          node_flux(j,k,l)=0.25_8*(mass_flux_x(j,k-1  ,l)+mass_flux_x(j  ,k,l)  &
                          +mass_flux_x(j+1,k-1,l)+mass_flux_x(j+1,k,l)) ! Mass Flux
        ENDDO
      ENDDO
    ENDDO
!$OMP END DO
      ...
      DO l=z_min,z_max+1
        DO k=y_min,y_max+1
          DO j=x_min-1,x_max+1
            sigma=ABS(node_flux(j,k,l))/(node_mass_pre(j+1,k,l))
            sigma2=ABS(node_flux(j,k,l))/(node_mass_pre(j,k,l))
            width=celldx(j)
            vdiffuw=vel1(j+1,k,l)-vel1(j+2,k,l)
            vdiffdw=vel1(j,k,l)-vel1(j+1,k,l)
            vdiffuw2=vel1(j,k,l)-vel1(j-1,k,l)
            vdiffdw2=-vdiffdw
            auw=ABS(vdiffuw)
            adw=ABS(vdiffdw)
            auw2=ABS(vdiffuw2)
            ...
            IF(node_flux(j,k,l).LT.0.0)THEN
              advec_vel(j,k,l)=vel1(j+1,k,l)+(1.0-sigma)*limiter
            ELSE
              advec_vel(j,k,l)=vel1(j,k,l)+(1.0-sigma2)*limiter2
            ENDIF
...
!$OMP           ,sigma2,limiter2,vdiffuw2,vdiffdw2,auw2,wind2)
      DO l=z_min,z_max+1
        DO k=y_min-1,y_max+1
          DO j=x_min,x_max+1
            sigma=ABS(node_flux(j,k,l))/(node_mass_pre(j,k+1,l))
            sigma2=ABS(node_flux(j,k,l))/(node_mass_pre(j,k,l))
            width=celldy(k)
            vdiffuw=vel1(j,k+1,l)-vel1(j,k+2,l)
            vdiffdw=vel1(j,k,l)-vel1(j,k+1,l)
            vdiffuw2=vel1(j,k,l)-vel1(j,k-1,l)
            vdiffdw2=-vdiffdw
            auw=ABS(vdiffuw)
            adw=ABS(vdiffdw)
            auw2=ABS(vdiffuw2)
            ...
            IF(vdiffuw2*vdiffdw2.LE.0.0) limiter2=0.0
            IF(node_flux(j,k,l).LT.0.0)THEN
              advec_vel(j,k,l)=vel1(j,k+1,l)+(1.0-sigma)*limiter
            ELSE
              advec_vel(j,k,l)=vel1(j,k,l)+(1.0-sigma2)*limiter2
            ENDIF
            mom_flux(j,k,l)=advec_vel(j,k,l)*node_flux(j,k,l)
